<?php
	...
	$sql= "SELECT COUNT(student_id) FROM student";
		...
		$sql= "SELECT COUNT(student_id) FROM answer";
		$query=mysqli_query($conn,$sql);
...
<table class="table">
	<tr>
		<td ><h3>Name of Professor: <?php echo $prof;?></h3></td>
	</tr>
	<tr>
		<td ><h3>Name of College: <?php echo $college_des;?> </h3></td>
	</tr>
	<tr>
	...
	<tr>
		<td ><h3>Rating Period: <?php echo $RP;?></h3></td>
	</tr>
</table>
	...
	<?php
		...
		foreach($conn->query("SELECT college_id, course_description, year_level, section_description, subj_code, AVG(answer) FROM answer WHERE ins_id='$prof_id' GROUP BY college_id, course_id, year_level, section_id, subj_code") as $row) {  
		echo "<tr>";  
		...
		if($sum_prof >= 95 && $sum_prof <= 100){
				...
				$sql="INSERT INTO average(ins_id, ins_name, average, rating, sem_id, school_year, rating_period) VALUES('$prof_id', '$prof', '$sum_prof','$rating_ave','$sem_id', '$SY', '$RP' )";
                $query=mysqli_query($conn, $sql);
                if($query){
                    echo "ok";
					}

				$sql="UPDATE instructor_evaluator SET activate='1' WHERE ins_id='$prof_id'";
		...
		?>
			...
			<form method="POST" role="form" action="result-average.php">
